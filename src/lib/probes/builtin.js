...
function semverRangeIncludes(spec, bad) {
  ...
  if (/[xX*]/.test(spec)) {
    const re = new RegExp(
      '^' + spec.split('.').map((p) => (/[xX*]/.test(p) ? '\\d+' : p.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))).join('\\.') + '($|[.\\-])'
    );
    return re.test(bad);
  }
...
function* _walkNpmLock(content) {
  ...
  if (pkg.packages) {
    ...
    for (const [key, entry] of Object.entries(pkg.packages)) {
      ...
      const chain = segments.slice(0, -1).map((s) => s.replace(/\/$/, '')).join(' > ') || 'root';
...
function* _walkYarnLock(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^[^\s#]/.test(line) && line.includes('@')) {
      // Header line.
      currentKeys = line.trim().replace(/:$/, '').split(/,\s*/).map((k) => k.replace(/^["']|["']$/g, ''));
...
export function probeCompromisedPackages(files) {
  ...
  files.forEach((file) => {
    ...
    let walker = null;
    if (/(^|\/)package-lock\.json$/.test(file.path) || /(^|\/)npm-shrinkwrap\.json$/.test(file.path))
      walker = _walkNpmLock;
