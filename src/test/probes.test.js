...
describe('probeCompromisedPackages', () => {
  ...
  // Depth round 5: SemVer-aware range matching (latent bug fix).
  it('flags axios via a caret range that contains 1.14.1 (^1.14.0)', () => {
    const pkg = JSON.stringify({ dependencies: { axios: '^1.14.0' } });
    expect(probeCompromisedPackages([file('package.json', pkg)])).toHaveLength(1);
  });

  it('flags axios via a tilde range that contains 1.14.1 (~1.14.0)', () => {
    const pkg = JSON.stringify({ dependencies: { axios: '~1.14.0' } });
    expect(probeCompromisedPackages([file('package.json', pkg)])).toHaveLength(1);
  });

  it('flags axios via x-wildcard (1.14.x)', () => {
    const pkg = JSON.stringify({ dependencies: { axios: '1.14.x' } });
    expect(probeCompromisedPackages([file('package.json', pkg)])).toHaveLength(1);
  });

  it('flags axios via ">=" comparator (>=1.14.0)', () => {
    const pkg = JSON.stringify({ dependencies: { axios: '>=1.14.0' } });
    expect(probeCompromisedPackages([file('package.json', pkg)])).toHaveLength(1);
  });

  it('does not flag axios via a tilde range that excludes 1.14.1 (~1.13.0)', () => {
    const pkg = JSON.stringify({ dependencies: { axios: '~1.13.0' } });
    expect(probeCompromisedPackages([file('package.json', pkg)])).toEqual([]);
  });

  it('warns on floating spec ("latest") when package has a known-bad version', () => {
    const pkg = JSON.stringify({ dependencies: { axios: 'latest' } });
    const f = probeCompromisedPackages([file('package.json', pkg)]);
    expect(f.length).toBeGreaterThan(0);
    expect(f[0].title).toMatch(/floating|latest|\*/i);
  });

  it('flags compromised version in optionalDependencies', () => {
    const pkg = JSON.stringify({ optionalDependencies: { axios: '1.14.1' } });
    expect(probeCompromisedPackages([file('package.json', pkg)])).toHaveLength(1);
  });

  it('flags compromised version in overrides', () => {
    const pkg = JSON.stringify({ overrides: { axios: '1.14.1' } });
    expect(probeCompromisedPackages([file('package.json', pkg)])).toHaveLength(1);
  });

  it("flags every version of an all-versions entry (versions: ['*'])", () => {
    const pkg = JSON.stringify({ dependencies: { 'plain-crypto-js': '2.3.1' } });
    const f = probeCompromisedPackages([file('package.json', pkg)]);
    expect(f).toHaveLength(1);
    expect(f[0].severity).toBe('critical');
  });

  it('flags a floating spec on an all-versions entry at critical, not high', () => {
    const pkg = JSON.stringify({ dependencies: { 'plain-crypto-js': 'latest' } });
    const f = probeCompromisedPackages([file('package.json', pkg)]);
    expect(f).toHaveLength(1);
    expect(f[0].severity).toBe('critical');
  });

  it('flags transitive compromised version in package-lock.json (npm v7+)', () => {
    const lock = JSON.stringify({
      lockfileVersion: 3,
      name: 'app',
      packages: {
        '': { name: 'app', version: '1.0.0' },
        'node_modules/something': { version: '1.0.0' },
        'node_modules/something/node_modules/axios': { version: '1.14.1' },
      },
    });
    const f = probeCompromisedPackages([file('package-lock.json', lock)]);
    expect(f).toHaveLength(1);
    expect(f[0].title).toMatch(/transitive/i);
  });

  it('flags compromised version in yarn.lock', () => {
    const yarnLock = `# THIS IS AN AUTOGENERATED FILE
"axios@^1.14.0", "axios@^1.14.1":
  version "1.14.1"
  resolved "https://registry.yarnpkg.com/axios/-/axios-1.14.1.tgz"
  integrity sha512-fake
`;
    const f = probeCompromisedPackages([file('yarn.lock', yarnLock)]);
    expect(f.length).toBeGreaterThan(0);
    expect(f.some((x) => x.evidence.includes('axios'))).toBe(true);
  });

  it('flags compromised version in pnpm-lock.yaml', () => {
...
`;
    const f = probeCompromisedPackages([file('pnpm-lock.yaml', pnpm)]);
    expect(f.length).toBeGreaterThan(0);
    expect(f.some((x) => x.evidence.includes('axios'))).toBe(true);
  });
});
