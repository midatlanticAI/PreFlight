// Phase 3 (Scala). XL-001 (ObjectInputStream/Jackson/Kryo), XL-002
// (Slick #${} / Anorm / Spark), XL-004 (trust-all), XL-006 (secrets).
// New detection, shadow only, family Learn pages inherited.
//
// Reference: docs/v05-research/preflight_v05_probe_inventory.md "12. Scala"
...
const IDS = ['SC-DESERIALIZE-001', 'SC-SECRETS-001', 'SC-SQL-RAW-001', 'SC-TLS-VERIFY-001'];
const ENTRIES = Object.values(PROBE_MANIFEST_V05).filter((e) => IDS.includes(e.probe_id));
const byId = (id) => PROBE_MANIFEST_V05[id];

describe('Phase 3 (Scala): manifest shape', () => {
  it('registers the four Scala adapters and they are the only scala ones', () => {
    const ids = Object.keys(PROBE_MANIFEST_V05);
    for (const id of IDS) expect(ids).toContain(id);
    const scIds = Object.values(PROBE_MANIFEST_V05)
      .filter((e) => e.language === 'scala')
      .map((e) => e.probe_id)
      .sort();
    expect(scIds).toEqual([...IDS].sort());
  });
  ...
  it('each Scala adapter resolves the right XL family + inherited Learn slug', () => {
    expect(byId('SC-DESERIALIZE-001').xl_family).toBe('XL-001');
    expect(byId('SC-DESERIALIZE-001').learn_more_slug).toBe('xl-unsafe-deserialization');
    expect(byId('SC-SQL-RAW-001').xl_family).toBe('XL-002');
    expect(byId('SC-SQL-RAW-001').learn_more_slug).toBe('xl-raw-query-interpolation');
    expect(byId('SC-TLS-VERIFY-001').xl_family).toBe('XL-004');
    expect(byId('SC-TLS-VERIFY-001').learn_more_slug).toBe('xl-tls-verification-disabled');
    expect(byId('SC-SECRETS-001').xl_family).toBe('XL-006');
...
describe('Phase 3 (Scala): fixture + Learn contracts', () => {
...
describe('Phase 3 (Scala): adapter detection (positive fires, negative silent)', () => {
  for (const e of ENTRIES) {
    describe(e.probe_id, () => {
